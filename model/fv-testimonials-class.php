...
//      $this->bUseTexy = ('yes' == strval( get_option( self::OPTION_TEXY ) )) ? true : false;
      ...
      if( $post_query->have_posts() ) {
        ...
        while ($post_query->have_posts()) : $post_query->the_post();
          if (($post->post_status == 'publish')&&($post->post_type == 'testimonial')) { 
            ...
            if( $template ) {
              $output = $objTestimonials->ParseTemplate(stripslashes($objTestimonials->aTemplates[$template]['content']), $aImages, $image, $testimonyTaxamony) . $strOutput;
            } else {
              $output .= '<div class="clsTestimonial"><h2><a name="'.$slug.'">'.get_the_title().'</a></h2>';
              $strImageTitle = get_the_title();
    ...
    if( $aInclude ) { 
      ...
      if( $post_query->have_posts() ) {
        ...
        while( $post_query->have_posts() ) : $post_query->the_post(); 
          ...
          if ($template) {
            $output .= $this->ParseTemplate(stripslashes($objTestimonials->aTemplates[$template]['content']), $aImages, $image, $testimonyTaxamony);
          } else {
            $output .= '<div class="clsTestimonial">
                          <h2><a name="'.$slug.'">'.get_the_title().'</a></h2>';
            $strImageTitle = get_the_title();
  ...
  private function ParseTemplate( $strTemplate, $aImages = false, $strSize = 'original' , $testimonyTaxamony = null ) {
    ...
    if (preg_match_all('/\[custom\-field\s*([\S]*?)( random)?\]/',$strTemplate, $matches)){      
      foreach( $matches[1] AS $key => $item ) { 
        if( isset($matches[2]) && isset($matches[2][0]) && $matches[2][0] == ' random' ) {      
          if( $strCustomField = get_post_meta($post->ID, $item) ) {  
            $strCustomField = $strCustomField[rand(0,count($strCustomField)-1)];
          }
        } else {
          $strCustomField = get_post_meta($post->ID, $item,true);
        }

        if ($strCustomField) {
          $strTemplate = preg_replace( '/\[custom\-field\s*'.$item.'( random)?\]/i', $strCustomField, $strTemplate );
        } else {
          $strTemplate = preg_replace( '/\[custom\-field\s*'.$item.'( random)?\]/i', '', $strTemplate );
        }         
      }
    }
    
    if (preg_match_all('/\[custom\-fields\s*([\S]*?)\]/',$strTemplate, $matches)){
      foreach( $matches[1] AS $key => $item ) {
        $aCustomField = get_post_meta($post->ID, $item);
        if ($aCustomField) {
          $sCustomFields = "<ul>\n"; 
          foreach( $aCustomField AS $sCustomField ) {
            $sCustomFields .= "<li>$sCustomField</li>\n"; 
          }
          $sCustomFields .= "</ul>\n"; 
          $strTemplate = preg_replace( '/\[custom\-fields\s*'.$item.'\]/i', $sCustomFields, $strTemplate );
        } else {
          $strTemplate = preg_replace( '/\[custom\-fields\s*'.$item.'\]/i', '', $strTemplate );
        }
      }
    }
    ...
    if( $aImages && count($aImages) ) {
      ...
      foreach( $aMatches as $aTextImage ) {
        ...
        if( 1 == $i ) {
          ...
          $strTemplate = preg_replace( '~\[1-image\](.*)\[end-1-image\]~imsU', $strText, $strTemplate, 1 );
          $strTemplate = preg_replace( '~\[image-1\](.*)\[/image-1\]~imsU', $strText, $strTemplate, 1 );          
        } else {
